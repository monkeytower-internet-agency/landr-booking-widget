...
import { apiLocale, browserLocale } from '@/lib/locale'
...
export function useBookingEstimate(
  ...
  // browserLocale() call site in the widget. Shaped through apiLocale()
  // (review round): the API's EstimateRequest.locale is max_length=16 and
  // only branches on the language[-region] subtags, so an extended
  // navigator.language value (script/extension subtags) gets trimmed down
  // instead of 422ing the request.
  const body = useMemo<EstimateRequestBody>(
    () => ({
      selected_days: selectedDays,
      participants_count: participantCount,
      addon_lines: addonLines,
      locale: apiLocale(browserLocale()),
    }),
